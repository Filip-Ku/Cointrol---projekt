...
export default function OutcomeScreen({ refresh, onRefresh,onGoalChange }) {
  const [title, setTitle] = useState('');
  const [amount, setAmount] = useState('');
  const [category, setCategory] = useState('');
  const [outcomeTransactions, setOutcomeTransactions] = useState([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [goalId, setGoalId] = useState(null);
  const [dateFilter, setDateFilter] = useState('all');

  const fetchOutcomeTransactions = () => {
    db.transaction(tx => {
      tx.executeSql(
        ...
        (tx, results) => {
         ...
         const filteredTransactions = filterTransactions(transactions);
                   setOutcomeTransactions(filteredTransactions);
        },
        (error) => {
  ...
  const filteredTransactions = outcomeTransactions.filter(transaction => {
  ...
  const handleSubmit = () => {
    ...
    if (title && amount && goalId) { // Ensure goalId is available
      db.transaction(tx => {
        tx.executeSql(
          'INSERT INTO transactions (date, title, amount, category, goal_id) VALUES (?, ?, ?, ?, ?)',
          [date, title, parseFloat(amount), "outcome", goalId], // Include goalId in the insert
          (tx, results) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.addTransaction}>
        <Text style={styles.addTransactionTitle}>Add Outcome</Text>
        <TextInput
          style={styles.input}
          placeholder="Title"
          value={title}
